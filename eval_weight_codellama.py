...
def main():
    ...
    for api, indices in tqdm(api_to_indices.items(), desc="Fitting distributions"):
        ...
        std = max(std, 1e-9)  # Prevent division by zero yielding NaN
        api_gmm_data[api] = {
            'mean': mean,
            'std': std,
        }
    ...
    results = {
        ...
        "D_test_U_dep": {
            ...
            "mean": float(np.nanmean(dep_weights)),
            "std": float(np.nanstd(dep_weights)),
            "min": float(np.nanmin(dep_weights)),
            "max": float(np.nanmax(dep_weights)),
            "median": float(np.nanmedian(dep_weights)),
            "weights": [float(w) for w in dep_weights],
        },
        "D_test_U_nondep": {
            "count": len(nondep_weights),
            "mean": float(np.nanmean(nondep_weights)),
            "std": float(np.nanstd(nondep_weights)),
            "min": float(np.nanmin(nondep_weights)),
            "max": float(np.nanmax(nondep_weights)),
            "median": float(np.nanmedian(nondep_weights)),
